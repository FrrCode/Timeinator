      ...
      {packedTimed.map((segment) => {
        ...
        return (
          <div
            ...
            //
            // pointer-events-none is a Timeinator change, not upstream: the day
            // column starts a drag only when the press lands on the column
            // itself (see its onPointerDown guard), and this wrapper is what a
            // press on a block actually hits. Leaving it interactive makes it
            // impossible to mark a slot somebody else has already marked, which
            // is the whole point of an availability poll. Nothing in a block is
            // clickable here -- clearing is a brush, not a click on a block.
            className="pointer-events-none absolute z-(--ec-z) min-h-(--ec-event-min-h,1.5rem) px-0.5 hover:z-40"
            style={
              {
